jQuery(document).ready(function($) {
	...
	// Validate project name
		...
		if (!is_projectname){
			...
			if (value.length < minLength){
				...
				// Disable submit button too
			    	$('#goforit').attr('disabled',true);
				$('.notice').replaceWith("<div class='notice notice-error is-dismissible'><p>Sorry, you project name cannot be over 30 characters long.</p></div>");
			} else {
	...
	// Validate ruling values
		...
		if (value.length < minLength){
			$('#rulevalueerror').append("<div id='errormessage'>Sorry, minimum length is 1 charachter</div>");
			// Disable submit button too
			$('#goforit').attr('disabled',true);
		} else if (value.length > maxLength){
			// Disable submit button too
			$('#goforit').attr('disabled',true);
			$('#rulevalueerror').append("<div id='errormessage'>Sorry, this value cannot be over 200 characters long.</div>");
		} else {
			$('#errormessage').remove();
			// Enable submit button
			$('#goforit').attr('disabled',false);
